...
void adicionarAlunoDisciplina(Aluno alunos[], int quantidadeAlunos, Turma turmas[], int &quantidadeTurmas) {
    ...
    cout << "Selecione a disciplina desejada pelo código:" << endl;
    ...
    for (int i = 0; i < quantidadeTurmas; ++i) {
        if (turmas[i].disciplina.codigo == codigoDisciplina) {
            indiceDisciplina = i;
            break;
        }
    }
    ...
    int indexAlunoDisciplina = turmas[indiceDisciplina].quantidadeAlunos;

    turmas[indiceDisciplina].alunos[indexAlunoDisciplina] = alunos[indiceAluno];
    turmas[indiceDisciplina].quantidadeAlunos++;

    cout << "Aluno adicionado à disciplina com sucesso!" << endl;
}
...
void registrarNotas(Turma turmas[], int quantidadeTurmas) {
    ...
    for (int i = 0; i < turmas[indiceDisciplina].quantidadeAlunos; i++) {
        cout << "Matrícula: " << turmas[indiceDisciplina].alunos[i].matricula << " - Nome: " << turmas[indiceDisciplina].alunos[i].nome << endl;
    }
    cout << "Digite a matrícula do aluno: ";
    cin >> matriculaAluno;
    ...
    int indiceAluno = -1;
    for (int i = 0; i < turmas[indiceDisciplina].quantidadeAlunos; ++i) {
        if (turmas[indiceDisciplina].alunos[i].matricula == matriculaAluno) {
            indiceAluno = i;
            break;
        }
    }
    ...
    cout << "Digite a qual nota deseja atribuir (1 ou 2): ";
    cin >> notaIndex;

    if (notaIndex < 1 || notaIndex > 2) {
    ...
    turmas[indiceDisciplina].notas[indiceAluno][notaIndex - 1] = nota;
